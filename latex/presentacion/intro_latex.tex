...
\begin{frame}[fragile]\frametitle{Incluir Gráficos en \LaTeX}
	\begin{itemize}
		...
		\begin{lstlisting}
\includegraphics[params]{nombreimagen}
		\end{lstlisting}
...
\begin{frame}[fragile]\frametitle{Operadores Matemáticos en \LaTeX}
	\begin{table}
		...
		\begin{tabular}{|l|c|c|c|}
			\hline
			\textbf{Categoría} & \textbf{Operadores} & \textbf{Código} & \textbf{Resultado}\\
			\hline
			F. Trigonomét. & \texttt{\textbackslash{}sin}, \texttt{\textbackslash{}cos}, \texttt{\textbackslash{}tan} & \texttt{\$\textbackslash{}cos(0) = 1\$} & $\cos(0) = 1$\\ 
			\hline
			Potencias, Indices & \texttt{\^}, \texttt{\_} & \texttt{\$k\_\{n+1\}=n\^{}\{10\}+k\_n\^{}2\$} & $k_{n+1}=n^{10}+k_n^2$\\
			\hline
			Fracciones & \texttt{\textbackslash{}frac} & \texttt{\$\textbackslash{}frac\{n!\}\{k!(n-k)!\}\$} & $\frac{n!}{k!(n-k)!}$\\
			\hline
			Raíces & \texttt{\textbackslash{}sqrt} & \texttt{\$\textbackslash{}sqrt[n]\{1+x\^{}2\}\$} & $\sqrt[n]{1+x^2}$\\
			\hline
			Sumas & \texttt{\textbackslash{}sum} & \texttt{\$\textbackslash{}sum\_\{i=1\}\^{}\{10\} t\_i\$} & $\sum_{i=1}^{10} t_i$\\
			\hline
			Integrales & \texttt{\textbackslash{}int} & \texttt{\$\textbackslash{}int\_0\^{}\textbackslash{}infty e\^{}\{-x\}\textbackslash{},dx\$} & $\int_0^\infty e^{-x}\,dx$\\
			\hline
		\end{tabular}
	\end{table}
\end{frame}
